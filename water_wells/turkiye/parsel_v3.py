...
def save_data():
    ...
    output_file_path = '/mnt/data/complete_parsel_data_bounds_partial.json'
    with open('complete_parsel_data_bounds_partial_2.json', 'w', encoding='utf-8') as output_file:
        json.dump(feature_collection, output_file, ensure_ascii=False, indent=4)
    print(f"Data saved to {output_file_path}")

def signal_handler(sig, frame):
    print('You pressed Ctrl+C! Saving data before exit...')
    save_data()
    sys.exit(0)
...
def main():
    ...
    signal.signal(signal.SIGINT, signal_handler)  # Ctrl + C yakalamak için
    ...
    for bounds in BOUNDS_LIST:
        ...
        for point in grid_points:
            ...
            if len(all_data) % 50 == 0:
                print("Sleeping for 30 seconds to avoid API rate limits...")
                time.sleep(30)

            # Her 500 istekte bir verileri kaydet
